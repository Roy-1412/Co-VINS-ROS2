...
void registerPub(rclcpp::Node::SharedPtr n)
{
    ...
    pub_agent_frame = n->create_publisher<agent_msg::msg::AgentMsg>("agent_frame", 1000);
...
void pubOdometry(const Estimator &estimator, const std_msgs::msg::Header &header)
{
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
    {
        ...
        odometry.child_frame_id = "zed_camera_link";
        Quaterniond tmp_Q;
...
{
    std::cout<<"calling preprocessagentframe"<<std::endl;
    double t_sec = estimator.Headers[WINDOW_SIZE - 2];
    int32_t sec = static_cast<int32_t>(std::floor(t_sec));
    uint32_t nsec = static_cast<uint32_t>((t_sec - sec) * 1e9);
    agent_frame_msg.header.stamp.sec = sec;
    agent_frame_msg.header.stamp.nanosec = nsec;
...
// if (!m_camera) {
//     std::cerr << "[ERROR] m_camera is null\n";
//     return;
// }
// std::cout << "[DEBUG] m_camera runtime type: " << typeid(*m_camera).name() << std::endl;

// if (auto pin = dynamic_cast<camodocal::PinholeCamera*>(m_camera.get())) {
//     const auto& params = pin->getParameters();
//     std::cout << "[PinholeCamera] fx=" << params.fx() << " fy=" << params.fy()
...
vector<BRIEF::bitset> brief_descriptors, window_brief_descriptors;
    ...
    for (int i = 0; i < (int)keypoints_uv.size(); i++)
    {
        ...
        // std::cout << "liftProjective [" << i << "] tmp_p = ("
        //       << tmp_p.x() << ", "
        //       << tmp_p.y() << ", "
        //       << tmp_p.z() << ")" << std::endl;
...
// std::cout << "keypoints_2d size = " << keypoints_2d.size() << std::endl;
// for (size_t i = 0; i < std::min<size_t>(5, keypoints_2d.size()); ++i) {
//     auto &pt = keypoints_2d[i].pt;
//     std::cout << "  keypoints_2d["<<i<<"].pt = ("
//               << pt.x << ", " << pt.y << ")" << std::endl;
// }
