        ...
        if (find_connection)
        {
            ...
            {  
                ...
                for (size_t i = 0; i < keyframe_vec.size(); i++)
                {
                    ...
                    if (need_update_path) 
                        {
                            std::cout << "need_update_path is true\n";
                        }
                        else
                        {
                            std::cout << "need_update_path is false\n";
                        }
                }
            }
        }
...
// void PoseGraph::pubLoopedCameraPose(const std_msgs::msg::Header &header, int sequence)
// {
//     // 1) 找最后一帧该序列的 KeyFrame
//     if (keyframe_vec.empty() || sequence_align_world.count(sequence) == 0)
//         return;

//     KeyFrame* last_kf = nullptr;
//     for (auto it = keyframe_vec.rbegin(); it != keyframe_vec.rend(); ++it) {
//         if ((*it)->sequence == sequence) {
//             last_kf = *it;
//             break;
//         }
...
//     // 2) 从 KeyFrame 读取回环优化后的全局位姿
//     Eigen::Vector3d P;
//     Eigen::Matrix3d R;
//     last_kf->getPose(P, R);

//     // 3) 构造 Odometry 消息（可选，若不需要 Odometry 可删掉这一块）
...
//     // 如果你有一个专门的 Odometry 发布器，可以在这里调用它：
//     // pub_looped_odom->publish(odom);
...
//     // {
//     //     sequence = iter->first; 
//     //     // 5) 根据 sequence 发到对应的 looped_pose 话题
//     //     switch (sequence) {
//     //     case 1:
//     //         posegraph_visualization->publish_by(pub_looped_pose1, header);
//     //         break;
//     //     case 2:
//     //         posegraph_visualization->publish_by(pub_looped_pose2, header);
//     //         break;
//     //     case 3:
//     //         posegraph_visualization->publish_by(pub_looped_pose3, header);
//     //         break;
//     //     case 4:
//     //         posegraph_visualization->publish_by(pub_looped_pose4, header);
//     //         break;
//     //     }
//     // }
