...
// 	matched_2d_cur_norm = feature_2d;
// 	matched_id = point_id;

// 	TicToc t_match;
// 	searchByBRIEFDes(matched_2d_old, matched_2d_old_norm, status, old_kf->feature_des, old_kf->keypoints, old_kf->keypoints_norm);
...
// 	    reduceVector(matched_3d, status);
// 	}
// 	std::cout << " 2.0 [findConnection] inlier count after PnP RANSAC: "
//           << matched_3d.size() << std::endl;
// 	if ((int)matched_3d.size() > MIN_LOOP_NUM)
// 	{
// 	    relative_t = PnP_R_old.transpose() * (origin_vio_T - PnP_T_old);
// 	    relative_q = PnP_R_old.transpose() * origin_vio_R;
