...
Estimator estimator;
static rclcpp::Logger g_logger = rclcpp::get_logger("vins_estimator");
...
queue<agent_msg::msg::AgentMsg> agent_msg_buf;
...
void agent_process()
{
    while(1)
    {
        ...
        if (pub_flag)
        {
            ...
            // if (image_buf.empty()) 
            // {
            // RCLCPP_WARN(g_logger, "image_buf is empty before popping");
            // } 
            ...
            //     if (front_t < tmp_t) 
            //     {
            //         RCLCPP_WARN(g_logger,"front timestamp %.9f < tmp timestamp %.9f: will pop", front_t, tmp_t);
            //     } 
            //     else 
            //     {
            //         RCLCPP_WARN(g_logger,"front timestamp %.9f >= tmp timestamp %.9f: will not pop", front_t, tmp_t);
            //     }
            // }
            ...
            {          
                ...
                // if (!image_msg) 
                // {
                //     RCLCPP_WARN(g_logger,"No image received: image_msg is NULL");
                // }
                // // 再判断时间戳是否匹配
                // else if (rclcpp::Time(image_msg->header.stamp).seconds() != rclcpp::Time(tmp_msg.header.stamp).seconds())
                // {
                //     RCLCPP_WARN(g_logger,"Timestamp mismatch: image time = %.9f, expected time = %.9f",
                //     rclcpp::Time(image_msg->header.stamp).seconds(),
                //     rclcpp::Time(tmp_msg.header.stamp).seconds());
                // }
                RCLCPP_WARN(g_logger, "can not find corresponding image");
            }
            else
            {
...
int main(int argc, char **argv)
{
    ...
    auto sub_feature = n->create_subscription<sensor_msgs::msg::PointCloud>("feature", rclcpp::QoS(rclcpp::KeepLast(2000)), feature_callback);
    auto sub_restart = n->create_subscription<std_msgs::msg::Bool>("restart", rclcpp::QoS(rclcpp::KeepLast(2000)), restart_callback);
    ...
    if (SWARM_AGENT)
    {
        RCLCPP_INFO(g_logger, "start swarm mode");
        // RCLCPP_INFO(g_logger, "IMAGE_TOPIC = %s", IMAGE_TOPIC.c_str());
        // RCLCPP_INFO(g_logger, "Resolved IMAGE_TOPIC = %s", n->get_node_topics_interface()->resolve_topic_name(IMAGE_TOPIC).c_str());

        //auto sub_image = n->create_subscription<sensor_msgs::msg::Image>(IMAGE_TOPIC, rclcpp::QoS(rclcpp::KeepLast(2000)), image_callback);
        
        m_camera = camodocal::CameraFactory::instance()->generateCameraFromYamlFile(config_file.c_str());
        std::cerr << "相机模型路径： " << config_file.c_str() << std::endl;
        std::string pkg_path = ament_index_cpp::get_package_share_directory("vins_estimator"); 
        BRIEF_PATTERN_FILE = pkg_path + "/support_files/brief_pattern.yml";
        RCLCPP_INFO(g_logger,"BRIEF_PATTERN_FILE = %s", BRIEF_PATTERN_FILE.c_str());
