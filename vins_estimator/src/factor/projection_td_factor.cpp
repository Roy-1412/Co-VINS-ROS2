...
Eigen::Matrix2d ProjectionTdFactor::sqrt_info = Eigen::Matrix2d::Identity();
double           ProjectionTdFactor::sum_t    = 0.0;
...
ProjectionTdFactor::ProjectionTdFactor(
  ...
  : pts_i(pts_i),
    pts_j(pts_j),
    // 把 2D velocity 扩到 3D
    velocity_i(velocity_i.x(), velocity_i.y(), 0.0),
    velocity_j(velocity_j.x(), velocity_j.y(), 0.0),
    td_i(td_i),
    td_j(td_j),
    // 居中行号
    row_i(row_i - ROW/2),
    row_j(row_j - ROW/2)
{
    ...
    if (a.isApprox(tmp)) tmp = Eigen::Vector3d(1,0,0);
    Eigen::Vector3d b1 = (tmp - a*(a.dot(tmp))).normalized();
...
void ProjectionTdFactor::check(double **parameters)const
{
